<template>
	<scroll-view scroll-y style="width: 100%; height: 100vh;" @scroll="rightScroll">
		<view class="content">
			...
			<view class="nav-tab"
				:style="{zIndex:99,position: 'fixed',backgroundColor:opacityNum?'#fff':'',paddingTop:navBarHeight + 'px'}">
				<view class="search-box">
					<image src="../../static/tabBar/search.png" class="icon-search"></image>
					<input type="text" placeholder="搜索咨询师" placeholder-class="placeholder_14" confirm-type="search"
			...
			<view class="centent">
				<consult :Data="primaryList[primaryIndex].child[secondIndex].child" v-if="activatIndex == 0"></consult>
			</view>
		</view>
	</scroll-view>
</template>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.primaryList = Data.applyJob
